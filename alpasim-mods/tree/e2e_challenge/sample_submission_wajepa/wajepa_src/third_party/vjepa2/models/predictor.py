...
from third_party.vjepa2.masks.utils import apply_masks
from third_party.vjepa2.utils.tensors import repeat_interleave_batch, trunc_normal_

from third_party.vjepa2.models.utils.modules import Block
...
class VisionTransformerPredictor(nn.Module):
    ...
    def forward(self, x, masks_x, masks_y, mod="video", mask_index=1):
        ...
        if self.chop_last_n_tokens > 0:
            if self.chop_last_n_tokens >= x.size(1):
                raise ValueError(
                    f"chop_last_n_tokens={self.chop_last_n_tokens} removes all "
                    f"{x.size(1)} predictor tokens"
                )
            x = x[:, : -self.chop_last_n_tokens]
            masks = masks[:, : -self.chop_last_n_tokens]
            argsort = argsort[:, : -self.chop_last_n_tokens]
        ...
        if not self.return_all_tokens:
            ...
            n_ctxt_kept = (argsort < N_ctxt).sum(dim=1)
            if not bool(torch.all(n_ctxt_kept == n_ctxt_kept[0]).item()):
                raise RuntimeError("chop_last_n_tokens produced uneven context lengths")
            x = x[:, int(n_ctxt_kept[0].item()) :, :]
            if x.size(1) == 0:
                raise RuntimeError("chop_last_n_tokens removed all predictor target tokens")
            ...
            if not bool(torch.all(n_ctxt_kept == n_ctxt_kept[0]).item()):
                raise RuntimeError("chop_last_n_tokens produced uneven context lengths")
            n_ctxt_kept_i = int(n_ctxt_kept[0].item())
            x_pred = x[:, n_ctxt_kept_i:, :]
            x_context = x[:, :n_ctxt_kept_i, :]
            if x_pred.size(1) == 0:
                raise RuntimeError("chop_last_n_tokens removed all predictor target tokens")
