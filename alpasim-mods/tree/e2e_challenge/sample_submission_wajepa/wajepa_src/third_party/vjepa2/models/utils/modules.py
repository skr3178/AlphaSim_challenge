...
def rotate_queries_or_keys(x, pos, n_registers, has_cls_first):
    ...
    emb_sin = emb_sin.repeat_interleave(2, dim=-1).to(dtype=x.dtype)
    emb_cos = emb_cos.repeat_interleave(2, dim=-1).to(dtype=x.dtype)
...
class RoPEAttention(nn.Module):
    def __init__(
        ...
        if pretrained_img_size is not None:
            if isinstance(pretrained_img_size, int):
                pretrained_img_size = (pretrained_img_size, pretrained_img_size)
            self.pretrained_grid_h = int(pretrained_img_size[0]) // patch_size
            self.pretrained_grid_w = int(pretrained_img_size[1]) // patch_size
        elif patch_size == 14:
            self.pretrained_grid_h = self.pretrained_grid_w = int(252 / patch_size)
        elif patch_size == 16:
            self.pretrained_grid_h = self.pretrained_grid_w = int(256 / patch_size)
        else:
            self.pretrained_grid_h = self.pretrained_grid_w = int(grid_size)
    ...
    def forward(
        ...
        if self.interpolate_rope:
            ...
            h_mask = h_mask * (self.pretrained_grid_h - 1) / max(H_patches - 1, 1)
            w_mask = w_mask * (self.pretrained_grid_w - 1) / max(W_patches - 1, 1)
