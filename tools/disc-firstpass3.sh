...
# MEASURED, not estimated. The 0.05 arm's attrib shows disc separates the top-2 candidates
# by 0.246 per unit w_disc (NOT the 0.85 implied by endpoint spread - the two candidates that
# compete for the win are similar to each other). Median decision margin is 0.0453, so
# w_disc ~ 0.18-0.2 is where disc first matches the margin. 0.02 and 0.05 are no-ops.
run_arm disc-d020-s1234 0.2 1234 || exit 1
run_arm disc-d040-s1234 0.4 1234 || exit 1
echo "=== DONE $(date +%H:%M:%S) - control + 0.05 + 0.2 + 0.4 ready to read ==="
